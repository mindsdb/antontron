...
def list_project_files(project_path: str, q: str | None = None, limit: int = 50):
...
def attach_project_file(request: ProjectFileAttachmentRequest):
    root = Path(request.project_path).expanduser().resolve()
    target = (root / request.path).resolve()
    try:
        target.relative_to(root)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail="Project file must be inside the selected project.") from exc
    if not target.exists() or not target.is_file():
        raise HTTPException(status_code=400, detail="Project file does not exist.")
    attachment_id = _new_id()
    target_dir = attachments_dir() / attachment_id
    target_dir.mkdir(parents=True, exist_ok=True)
    filename = _safe_name(target.name)
    copy_path = target_dir / filename
    shutil.copy2(target, copy_path)
    mime = mimetypes.guess_type(filename)[0]
    text, extraction_status, truncated, note = extract_attachment_text(copy_path, mime)
    metadata = {
        "id": attachment_id,
        "kind": "project_file",
        "name": filename,
