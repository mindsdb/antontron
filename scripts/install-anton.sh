...
case "$SOURCE" in
  local)
    ...
    echo "→ Installing anton[cowork-server] from local source at /build/anton" >&2
    pip install --no-cache-dir "/build/anton[cowork-server]"
    ;;
  git)
    REF="${ANTON_VERSION:-main}"
    echo "→ Installing anton[cowork-server] from git+ssh://git@github.com/mindsdb/anton.git@${REF}" >&2
    ...
    pip install --no-cache-dir "anton[cowork-server] @ git+ssh://git@github.com/mindsdb/anton.git@${REF}"
    ;;
  *)
...
python3 -c "import anton.cli, anton.chat, anton.cowork.server.main" 2>/dev/null \
